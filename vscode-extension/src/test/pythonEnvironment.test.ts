...
describe('PythonEnvironment', () => {
    ...
    describe('PubMed package policy', () => {
        it('should pin Zotero Keeper installs to the fixed extension source archive', () => {
            ...
            assert.strictEqual(ZOTERO_KEEPER_VERSION, '2.3.1');
        });
        ...
        it('should compare version baselines numerically', () => {
            assert.strictEqual(compareDottedVersions('0.7.2', PUBMED_SEARCH_VERSION) < 0, true);
            assert.strictEqual(compareDottedVersions(PUBMED_SEARCH_VERSION, '0.7.3'), 0);
            assert.strictEqual(compareDottedVersions('0.7.4', PUBMED_SEARCH_VERSION) > 0, true);
        });
    });
