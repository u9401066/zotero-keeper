...
class TestUvDownloadUrls(unittest.TestCase):
    ...
    def test_url_version_consistency(self):
        ...
        for key, url in UV_DOWNLOADS.items():
            self.assertIn(
                UV_VERSION, url, f"{key} URL doesn't contain version {UV_VERSION}"
            )
...
class TestPathConstruction(unittest.TestCase):
    ...
    def test_python_binary_path_unix(self):
        ...
        self.assertTrue(
            expected.endswith("bin/python") or expected.endswith("bin\\python")
        )
...
class TestMacPythonPathDiscovery(unittest.TestCase):
    ...
    def test_discovery_order_prefers_homebrew(self):
        ...
        homebrew_idx = next(
            i for i, p in enumerate(self.KNOWN_MAC_PATHS) if "homebrew" in p
        )
        usr_bin_idx = next(
            i for i, p in enumerate(self.KNOWN_MAC_PATHS) if p == "/usr/bin/python3"
        )
...
class TestSourceCodePatterns(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cline_path = os.path.join(base, "src", "clineMcpConfig.ts")
        zk_pkg_path = os.path.join(base, "src", "zoteroKeeperPackage.ts")
        ...
        with open(cline_path, "r", encoding="utf-8") as f:
            cls.cline_source = f.read()
        with open(zk_pkg_path, "r", encoding="utf-8") as f:
            cls.zk_pkg_source = f.read()
    ...
    def test_uv_recreates_invalid_venv_before_setup(self):
        ...
        self.assertIn("await this.killPythonProcesses()", self.uv_source)
    ...
    def test_uv_tracks_install_state_for_migration(self):
        ...
        self.assertIn("direct_url.json", self.uv_source)
        self.assertIn("SOURCE_URLS", self.uv_source)

    def test_uv_force_reinstalls_direct_url_packages(self):
        """Direct URL package refresh must force reinstall same-version archives."""
        self.assertIn("--force-reinstall", self.uv_source)
        self.assertIn("ZOTERO_KEEPER_PACKAGE", self.uv_source)
        self.assertIn("ZOTERO_KEEPER_SOURCE_URL", self.uv_source)

    def test_uv_stops_old_mcp_processes_before_upgrade(self):
        """Package refresh should stop old venv-backed MCP processes before replacing files."""
        upgrade_start = self.uv_source.index(
            "async upgradePackages(): Promise<boolean>"
        )
        upgrade_body = self.uv_source[
            upgrade_start : self.uv_source.index("return true;", upgrade_start)
        ]
        self.assertIn("await this.killPythonProcesses()", upgrade_body)
        self.assertIn("await this.installPackages", upgrade_body)

    def test_uv_powershell_process_kill_uses_literal_path_pattern(self):
        """PowerShell fallback should compare real Windows paths, not double-escaped paths."""
        self.assertIn("quotePowerShellSingleQuoted(`${this.venvDir}*`)", self.uv_source)
        self.assertNotIn("this.venvDir.replace(/\\\\/g, '\\\\\\\\')}*", self.uv_source)
    ...
    def test_py_system_installs_use_managed_venv(self):
        """System/custom Python package installs must not target system site-packages."""
        self.assertIn("system-python-venv", self.py_source)
        self.assertIn("ensureWritablePackageEnvironment", self.py_source)
        self.assertIn("ZOTERO_KEEPER_PACKAGE", self.py_source)
        self.assertIn("direct_url.json", self.py_source)
        self.assertIn("--force-reinstall", self.py_source)
        self.assertIn(
            "Using uv for package installation in an isolated environment",
            self.py_source,
        )

    def test_py_uv_lookup_is_not_extension_storage_only(self):
        """Manual/system mode should find uv from PATH and common user install paths."""
        self.assertIn("resolveUvPath", self.py_source)
        self.assertIn("AppData", self.py_source)
        self.assertIn("/opt/homebrew/bin/uv", self.py_source)
    ...
    def test_ext_resyncs_cline_on_configuration_changes(self):
        """Cline MCP settings should refresh when zoteroMcp settings or workspace folders change."""
        self.assertIn("onDidChangeConfiguration", self.ext_source)
        self.assertIn("affectsConfiguration('zoteroMcp')", self.ext_source)
        self.assertIn("onDidChangeWorkspaceFolders", self.ext_source)
        self.assertIn(
            "syncRuntimeConsumers(context, resolvedPythonPath)", self.ext_source
        )
    ...
    def test_cline_preserves_user_local_settings(self):
        ...
        self.assertIn("mergeManagedEntry", self.cline_source)
        self.assertIn("disabled: existing.disabled ?? next.disabled", self.cline_source)
        self.assertIn("alwaysAllow: existing.alwaysAllow", self.cline_source)
    ...
    def test_zotero_keeper_package_uses_extension_archive(self):
        ...
        self.assertIn("v0.5.28-ext.tar.gz", self.zk_pkg_source)
    ...
    def test_python_version_consistent(self):
        ...
        py_match = re.search(
            r"const MIN_PYTHON_VERSION = \[(\d+), (\d+)\]", self.py_source
        )
        self.assertIsNotNone(uv_match, "PYTHON_VERSION not found in uvPythonManager.ts")
        self.assertIsNotNone(
            py_match, "MIN_PYTHON_VERSION not found in pythonEnvironment.ts"
        )
