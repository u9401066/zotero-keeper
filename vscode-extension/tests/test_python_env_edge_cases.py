...
DEFAULT_RELEASE_ZOTERO_KEEPER_PACKAGE = (
    ...
    "v0.8.0-ext.tar.gz#subdirectory=mcp-server"
)
PUBMED_SEARCH_FIXED_COMMIT = "febf53a8ff1ee253a625869ba251365f73a23c68"
...
def get_uv_path() -> str:
    ...
    try:
        result = subprocess.run(
            ["where", "uv"], capture_output=True, text=True, timeout=10
        )
...
class TestPythonEnvEdgeCases:
    ...
    def _install_packages(
        self, venv_dir: Path, packages: list[str]
    ) -> tuple[bool, str]:
    ...
    def _check_installed_version_via_pip(
        self, venv_dir: Path, pkg_name: str
    ) -> Optional[str]:
    ...
    def test_B1_system_python_exists(self):
        ...
        is_different = os.path.normpath(uv_executable) != os.path.normpath(
            sys.executable
        )
        record(
            "B1_system_python_coexist",
            is_different,
    ...
    def test_C3_corrupted_python_binary(self):
        ...
        try:
            result = run_cmd([str(python), "--version"])
            reports_python = (
                result.returncode == 0 and result.stdout.strip().startswith("Python")
            )
        except (OSError, subprocess.SubprocessError):
            reports_python = False
        log(
            f"Fixed checkReadySync would report ready: {reports_python} (should be False)"
        )
        ...
        record(
            "C3_corrupted_python",
            is_broken and not reports_python and fixed,
    ...
    def test_C4_venv_exists_packages_missing(self):
        ...
        record(
            "C4_missing_packages_detect",
            not can_import and can_import_after,
    ...
    def test_D3_version_check_accuracy(self):
        ...
        for pkg_name, import_name, min_ver in [
            ("zotero-keeper", "zotero_mcp", "2.2.0"),
            ("pubmed-search-mcp", "pubmed_search", "0.6.3"),
        ]:
            ...
            meta_ver = (
                meta_result.stdout.strip() if meta_result.returncode == 0 else None
            )
            ...
            if attr_ver != pip_ver:
                log(
                    "  [INFO] __version__ mismatch (known issue, no longer used)",
                    "WARN",
                )

            if meta_ver != pip_ver:
                issues.append(
                    f"importlib.metadata({meta_ver}) != pip({pip_ver}) for {pkg_name}"
                )
                _all_ok = False  # noqa: F841
            ...
            if "OUTDATED" in ver_check_result:
                issues.append(
                    f"Fixed check still reports OUTDATED for {pkg_name}: {ver_check_result}"
                )
                _all_ok = False  # noqa: F841
    ...
    def test_D4_already_latest_upgrade(self):
        ...
        record(
            "D4_already_latest",
            result.returncode == 0 and ver1 == ver2,
...
"""
    ...
    def test_H2_concurrent_venv_creation(self):
        ...
        record(
            "H2_rapid_venv_creation",
            ok1 and ok2 and result.returncode == 0,
