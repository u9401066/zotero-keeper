...
### 步驟 1: 了解研究問題
先由 Agent 從使用者問題提取 PICO，再用 `validate_pico_plan(description=question, p=..., i=..., c=..., o=...)` 驗證計畫（工具不代替 Agent 解析自然語言）。將回傳的 pipeline 傳給 `unified_search(query=question, pipeline=plan["pipeline"])`：
- **P**opulation: 研究對象
- **I**ntervention: 介入措施
- **C**omparison: 對照組
- **O**utcome: 結果指標
...
| 工具 | 何時使用 |
|------|----------|
| `read_session(request={"action":"pmids"})` | 需要取得之前搜尋的 PMID |
| `read_session(request={"action":"log"})` | 查看或重讀 session 內已持久化的搜尋與操作紀錄 |
| `read_session(request={"action":"article","pmid":"..."})` | 取得已快取的文章詳情（避免重複 fetch） |
...
所有 confirmed mutation 全部必須帶已核准 proposal 中的
...
Copilot 動作:
1. Agent 先拆解 PICO，再以 validate_pico_plan 驗證並產生 pipeline
