...
- Release baseline: VSIX 0.7.0 installs Zotero Keeper 2.1.0 (32 default tools, 6 concrete resources) and PubMed Search MCP 0.6.1 at `ad85dde` (45 tools, 16 categories).
...
| Create collection/search/note, update safe metadata, add collection membership | UI only | Confirmed Local API v3 tools |
| Attach a file to an existing parent / write indexed full text | UI or external workflow | Confirmed Local API v3 tools |
...
1. Before preview, obtain a response-bound `server_id` from an exact Local API
   read or `authorize_local_writes`. For `update_item_fields`, use the object
   version from that exact-item response. For `set_attachment_fulltext`, use the
   response-bound library cursor, not the attachment object version.
...
4. If not already authorized, call
   ...
   `attach_file_to_item`, use `require_remembered=true` and grant **Always Allow**.
...
`update_item_fields` uses the exact item's response-bound object version;
`set_attachment_fulltext` uses a response-bound library version and bulk
...
`add_items_to_collection` is additive: it validates the exact collection and all one-to-50 items before one batch write, preserves all existing memberships, and reports per-item partial failures. It is not a move-to-root primitive. Keeper 2.1.0 exposes no raw delete MCP tool; use Zotero's UI for destructive cleanup.

`attach_file_to_item` implements Zotero's [three-phase stored-file upload](https://www.zotero.org/support/dev/web_api/v3/file_upload). A failure after child creation may return `partial=true` and `attachment_key`; inspect the item in Zotero rather than blindly retrying.
...
- Keeper: `uv run python tests/check_mcp.py` should discover the SDK v2 server surface; the default server has 32 tools, 6 concrete resources, and 4 URI templates.
